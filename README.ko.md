...
현재 31개 테스트가 수집기 개인정보 보호, 반복 가능한 훅 병합, 이벤트 정제, 불투명 ID, 직원 퇴장·유령 직원 정리·기능 담당 이름, 안정적인 프로젝트 순서, 프로젝트 종료·재개, 상태 오래됨 경계, 협업 추론, localhost 전용 서버, 회전 이벤트 파일, 정적 호스팅 산출물을 검증합니다. 단계별 개발 과정과 결과는 [docs/DEVELOPMENT.ko.md](docs/DEVELOPMENT.ko.md)에 정리되어 있습니다.
