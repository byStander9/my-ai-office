...
- 로컬에서 명시적으로 켠 경우에만 마스킹된 지시·협업 내용을 표시하고, 서브에이전트 둘 이상이 협업할 때 회의 테이블에 최근 논의 표시
...
이벤트 수집기는 허용 목록 방식으로 동작합니다. 기본값에서는 프롬프트, 명령, 도구 입출력, 파일 내용, 자격 증명, 전체 작업 경로, 원본 세션·턴·직원·도구 실행 ID를 저장하지 않습니다. 로컬 상세 기록을 명시적으로 켠 경우에만 짧게 마스킹한 사용자 지시와 허용된 서브에이전트 협업 도구의 `message` 필드를 추가 저장합니다. 셸·패치 입력, 임의 도구 응답, transcript, 서브에이전트 최종 메시지는 저장하지 않습니다. 브라우저 API에서도 다시 허용 목록과 마스킹을 적용합니다.
...
현재 38개 테스트가 수집기 개인정보 보호·선택적 상세 마스킹, 단순 채팅·오래된 프로젝트 필터, 반복 가능한 훅 병합, 이벤트 정제, 불투명 ID, 장기 업무 활동 요약, 직원 퇴장·유령 직원 정리·기능 담당 이름, 안정적인 프로젝트 순서, 프로젝트 종료·재개, 상태 오래됨 경계, 협업 논의, localhost 전용 서버, 회전 이벤트 파일, 정적 호스팅 산출물을 검증합니다. 단계별 개발 과정과 결과는 [docs/DEVELOPMENT.ko.md](docs/DEVELOPMENT.ko.md)에 정리되어 있습니다.
